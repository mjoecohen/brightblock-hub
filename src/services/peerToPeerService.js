...
const peerToPeerService = {
  ...
  sendPeerSignal: function(signal) {
    ...
    if (signal.type.indexOf("-adm") > 1) {
      let peer = store.getters["onlineAuctionsStore/getAdministrator"](
        signal.data.auctionId
      );
      if (!peer || !peer.connection) {
        throw new Error(
          "No administrator present - please wait for the administrator to join the session."
        );
      }
