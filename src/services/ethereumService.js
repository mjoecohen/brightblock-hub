...
const ethereumService = {
  ...
  connectToBlockChain: function(clientState) {
    ...
    return new Promise(resolve => {
      if (
        ethereumService.accounts &&
        ethereumService.artmarketContract &&
        ethereumService.myContract
      ) {
      ...
      web3.eth.getAccounts(function(error, result) {
        ...
        ethereumService.artmarketContract = web3.eth.contract(abi);
        ethereumService.myContract = ethereumService.artmarketContract.at(
  ...
  setPriceOnChain: function(priceData, success, failure) {
    ...
    web3.eth.getAccounts(function(error, result) {
      if (error || !result || (result.length === 0)) {
        ...
        web3.eth.defaultAccount = result[0];
        ethereumService.myContract.sell(
          priceData.itemIndex,
          priceData.amountInWei,
          function(err, txId) {
            if (err) {
              console.log(err);
              failure({ failed: true, message: err });
  ...
  purchase: function(priceData, success, failure) {
    ethereumService.loggedIn(
      function() {
        ethereumService.myContract.buy(
          priceData.itemIndex,
          priceData.buyer,
